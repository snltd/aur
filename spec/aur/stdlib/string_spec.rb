...
class StringTest < MiniTest::Test
  def test_to_safe
    ...
    assert_equal('r-e-m', 'R.E.M.'.to_safe)
    assert_equal('20_cases_suggestive_of', '20 Cases Suggestive of ...'.to_safe)
    assert_equal('v-vm', 'V/Vm'.to_safe)
    assert_equal('say_yes', 'Say "Yes!"'.to_safe)
    assert_equal('blank--logotone', '____________ (Logotone)'.to_safe)
    assert_equal('soho--st_ives--tangier', 'Soho > St. Ives > Tangier'.to_safe)
    assert_equal('baby_one_more_time', '...Baby One More Time'.to_safe)
    assert_equal('simple-string', 'simple-String'.to_safe)
    assert_equal('simple_string', 'Simple String'.to_safe)
    assert_equal('incident_at_23rd', 'Incident @ 23rd'.to_safe)
    assert_equal('plus2', '+2'.to_safe)
