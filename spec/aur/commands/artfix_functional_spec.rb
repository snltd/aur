...
class TestArtfixCommand < Minitest::Test
  ...
  def test_artfix_noop
    assert_output(expected_output(RES_DIR.join('artfix')), '') do
      Aur::Action.new(
        :artfix,
        [],
        '<directory>': [RES_DIR.join('artfix')], noop: true
      ).run!
    end

    assert RES_DIR.join('artfix/albums/jesus_lizard.liar/cover.jpg').exist?
    refute RES_DIR.join('artfix/albums/jesus_lizard.liar/front.jpg').exist?
  end

  def test_artfix
    with_test_file(RES_DIR.join('artfix')) do |dir|
      assert_output(expected_output(dir), '') do
        Aur::Action.new(:artfix, [], { '<directory>': [dir] }).run!
      end

      refute dir.join('albums/jesus_lizard.liar/cover.jpg').exist?
      assert dir.join('albums/jesus_lizard.liar/front.jpg').exist?
